...
export default function MesExercicesScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await api.get('/enseignant/epreuves/', { params: { type_epreuve: 'exercice' } });
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.subtitle}>{items.length} exercice(s)</Text>
        </View>
      </View>
            ...
            return (
              <TouchableOpacity style={styles.card} activeOpacity={0.85} onPress={() => router.push(`/enseignant/exercices/${item.id_epreuve}` as any)}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={styles.metaItem}><Ionicons name="help-circle-outline" size={13} color={colors.textLight} /><Text style={styles.metaText}>{item.nb_questions_detail} question(s)</Text></View>
                </View>
      ...
      <TouchableOpacity style={styles.fab} onPress={() => router.push('/enseignant/exercices/nouveau' as any)} activeOpacity={0.9}>
        ...
        <Text style={styles.fabText}>Nouvel exercice</Text>
      </TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  metaItem: { flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 8 },
